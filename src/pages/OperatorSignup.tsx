...
const OperatorSignUp = () => {
  return (
    <div className="flex flex-row h-screen">
      ...
      <div className="flex flex-col w-[70vw] min-[1550px]:px-21.7 min-[1200px]:px-18 px-13 min-[1550px]:pt-20 pt-14">
        ...
        <div className="flex flex-row justify-between w-full min-[1550px]:mb-6 min-[1200px]:mb-8 mb-6">
          ...
          />
          <Field heading="Email" type="email" className="self-start" />
        </div>
        ...
        <button className="rounded-full bg-operator-signup shadow-operator-signup-button text-white min-[1550px]:text-xl min-[1200px]:text-base text-base font-bold w-full min-[1550px]:h-13.2 h-10 mt-3.2">
          註冊
        </button>
...
const Field = ({ heading, type, className, downText }: FieldProps) => {
  return (
    <div className={classNames('relative flex flex-col w-[45%]', className)}>
      <Heading heading={heading} />
      <input type={type} className={classNames(Style, 'w-full')} />
      <p className="text-grey ml-5.5 mt-2 w-[100%] min-[1500px]:text-base min-[1200px]:text-sm text-[12px]">
        {downText}
      </p>
    </div>
