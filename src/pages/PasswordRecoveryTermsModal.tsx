        ...
        <DialogContent>
          ...
          >
            ...
            <div className="yellowScroll overflow-y-scroll h-[350px] min-[1400px]:h-[600px]  w-full">
              <Box sx={{ background: '#F5F5F5', width: '98%', p: '11px' }}>
                <div className="text-dark-grey font-bold leading-5">
                  ...
                  {PASSWORD_RECOVERY_TERMS_AND_CONDITIONS.map(({ title, content }, index) => (
                        ...
                        {content.map((item, itemIndex) => (
                          <li
                            ...
                            className={classNames({
                              'ml-[24px]': item.isListDisc,
                              'ml-0': !item.isListDisc
                            })}
                          >
                            {item.text}
                          </li>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', width: '95%' }}>
              <Link
                className="rounded-md text-navy-blue bg-white-smoke font-bold shadow-download-btn text-lg mt-3 px-10 self-end flex gap-2 items-center"
                to="/download.docx"
                target="_blank"
                download="土星_平台條款內容"
              >
