...
function ProgressBar({steps,stepNumber,stepName}) {
  

  const noOfSteps = Array.from(
    { length: steps },
    (value, index) => index
  );
  return (
        ...
        {noOfSteps.map((step, index) => {
          ...
          return (
            <div className="flex flex-col relative items-center">
              {stepNumber==index+1 && <p className="text-navy-blue text-2.5xl absolute w-max -mt-10">{stepName}</p>}
