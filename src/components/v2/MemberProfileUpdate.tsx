...
const Schema = yup
  .object({
    ...
    password_update_able: yup.boolean().default(false),
    password1: yup
      .string()
      ...
      {infoUpdateAble && (
        <div className="flex justify-between w-full">
          <CustomButton
            children="刪除帳號"
            className="border !border-silverstone !text-silverstone !bg-transparent min-[1600px]:text-lg min-[1500px]:text-base min-[1300px]:text-sm text-xms font-bold rounded-mdlg min-[1600px]:w-[154px] min-[1500px]:w-[125px] min-[1300px]:w-[105px] w-[95px]  min-[1600px]:h-10.5 min-[1500px]:h-9 min-[1300px]:h-8 h-7"
            onClick={() => setValue('password_update_able', true)}
          />
...
const PasswordInput = ({ className, register, id, errors }: PasswordInputProps) => {
  ...
  return (
    <>
      ...
      {errors && errors[id] && <p className="text-bright-red text-xs mt-1 ml-2">{errors[id]?.message?.toString()}</p>}
    </>
  );
};
